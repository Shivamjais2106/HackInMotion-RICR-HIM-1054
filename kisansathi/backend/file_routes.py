...
def upload_video():
    ...
    try:
        ...
        if result["success"]:
            return jsonify({"message": "Video uploaded successfully", "data": result}), 200
        else:
            return jsonify({"error": result["error"]}), 500
...
def upload_document():
    ...
    try:
        ...
        if result["success"]:
            return jsonify({"message": "Document uploaded successfully", "data": result}), 200
        else:
            return jsonify({"error": result["error"]}), 500
...
def optimize_image(public_id):
    ...
    try:
        ...
        return jsonify({"url": url, "width": width, "height": height}), 200
...
def get_thumbnail(public_id):
    ...
    try:
        ...
        return jsonify({"url": url, "size": size}), 200
