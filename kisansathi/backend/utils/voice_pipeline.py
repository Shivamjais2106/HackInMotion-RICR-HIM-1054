...
def generate_fertilizer_explanation(fertilizer, crop, nitrogen, phosphorus, potassium, soil_type):
    ...
    details = fertilizer_details.get(
        fertilizer,
        {
    ...
    explanation = (
        f"For your {crop} crop with nitrogen level {nitrogen} mg/kg, "
        f"phosphorus {phosphorus} mg/kg, and potassium {potassium} mg/kg "
        f"in {soil_type} soil, the recommended fertilizer is {fertilizer}, "
        f"which is a {details['description']}.\n\n"
        f"This fertilizer {details['benefits']}. The NPK ratio is {details['npk']}.\n\n"
        f"Application method: {details['application']}.\n\n"
        f"Given your soil conditions with {soil_type} soil type, this fertilizer will help "
        "optimize nutrient availability and improve crop yield. "
