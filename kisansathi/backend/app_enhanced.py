...
_raw_origins = os.getenv("ALLOWED_ORIGINS", "http://localhost:3000,http://localhost:5173,http://localhost:8080")
...
def health_check():
    ...
    return jsonify(
        {"status": "healthy", "timestamp": datetime.now().isoformat(), "database": "MongoDB", "version": "4.0.0"}
    ), 200
...
def get_groups():
    ...
    return jsonify({"groups": groups, "total": len(groups), "cached": True}), 200
...
def create_group():
    ...
    return jsonify(
        {"message": "Group created successfully", "group_id": str(result.inserted_id), "group": group_doc}
    ), 201
...
def send_message(group_id):
    ...
    message_doc = {
        "group_id": group_oid,
        "sender": {"id": ObjectId(user_id), "name": user["name"], "avatar": data.get("avatar", "👤")},
        "text": data.get("text", ""),
    ...
    db["groups"].update_one(
        {"_id": group_oid},
        {"$set": {"lastMessage": data.get("text", "Image shared"), "updated_at": datetime.now().isoformat()}},
    )
    ...
    return jsonify(
        {"message": "Message sent successfully", "message_id": str(result.inserted_id), "data": message_doc}
    ), 201
        ...
        return jsonify(
            {
                ...
                "member": {"id": str(member_oid), "name": member["name"], "mobile": member["mobile"]},
            }
        ), 200
    else:
...
def make_admin(group_id):
    ...
    if member_oid not in group["admins"]:
        db["groups"].update_one(
            {"_id": group_oid}, {"$push": {"admins": member_oid}, "$set": {"updated_at": datetime.now().isoformat()}}
        )
        ...
        return jsonify(
            {
                ...
                "admin": {"id": str(member_oid), "name": member["name"], "mobile": member["mobile"]},
            }
        ), 200
    else:
...
def advanced_crop_recommendation():
    ...
    try:
        ...
        if recommendations:
            ...
            return jsonify({"success": False, "error": "No recommendations found", "recommendations": []}), 400
...
def get_seasons():
    ...
    try:
        ...
        return jsonify({"seasons": seasons_list, "total": len(seasons_list)}), 200
    except Exception as e:
...
def get_available_crops():
    ...
    try:
        ...
        return jsonify({"success": True, "crops": available_crops, "total": len(available_crops)}), 200
    except Exception as e:
...
def get_farmer_crops(farmer_id):
    ...
    try:
        ...
        return jsonify({"success": True, "crops": crops, "total": len(crops)}), 200
    except Exception as e:
...
def regenerate_crop_reminders(crop_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": f"Regenerated {len(reminders)} reminders", "reminders_created": len(reminders)}
        ), 200
    except Exception as e:
...
def get_crop_reminders(crop_id):
    ...
    try:
        ...
        return jsonify({"success": True, "reminders": reminders, "total": len(reminders)}), 200
    except Exception as e:
...
def complete_reminder():
    ...
    try:
        ...
        db["reminders"].update_one(
            ...
            {"$set": {"completed": True, "notes": notes, "completed_at": datetime.now().isoformat()}},
        )

        logger.info(f"Reminder {reminder_id} marked as completed")
        return jsonify({"success": True, "message": "Reminder marked as completed"}), 200
    except Exception as e:
...
def get_crop_photos(crop_id):
    ...
    try:
        ...
        return jsonify({"success": True, "photos": photos, "total": len(photos)}), 200
    except Exception as e:
...
def upload_crop_photo(crop_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "photo_id": str(result.inserted_id), "message": "Photo uploaded successfully"}
        ), 201
    except Exception as e:
