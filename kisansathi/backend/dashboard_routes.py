...
def get_endpoint_stats():
    ...
    return jsonify(
        {"timestamp": datetime.now().isoformat(), "endpoints": performance_monitor.get_endpoint_stats()}
    ), 200
...
<<<<<<< HEAD
                    with open(filepath, "r") as f:
                        ...
                        logs.append({"file": filename, "lines": lines})
    except Exception as e:
        logs = [{"error": str(e)}]

    return jsonify({"timestamp": datetime.now().isoformat(), "logs": logs}), 200
...
<<<<<<< HEAD
    ...
    if stats["error_rate"] > 5:
        alerts.append(
            {"type": "performance", "severity": "medium", "message": f"High error rate: {stats['error_rate']:.2f}%"}
        )
    ...
    return jsonify({"timestamp": datetime.now().isoformat(), "alerts": alerts, "alert_count": len(alerts)}), 200
