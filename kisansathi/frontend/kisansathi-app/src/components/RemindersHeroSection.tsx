      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-16">
        <div className="text-center mb-16">
          <h1 className="text-5xl md:text-6xl font-bold text-white mb-6 leading-tight">
            {language === "en" ? "Never Miss a Farm Task Again" : "कभी भी खेत का काम न भूलें"}
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-16">
          {[
            ...
            <div
              key={index}
              className="bg-white/10 backdrop-blur-md rounded-lg p-6 text-white border border-white/20"
            >
              <div className="text-blue-300 mb-3">{stat.icon}</div>
              <h3 className="text-xl font-bold mb-2">
                {language === "en" ? stat.titleEn : stat.titleHi}
              </h3>
              <p className="text-blue-100">{language === "en" ? stat.descEn : stat.descHi}</p>
            </div>
          ))}
        </div>
      </div>
