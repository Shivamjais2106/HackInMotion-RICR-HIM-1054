...
function DiseaseDetection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${baseURL}/disease-predict`, {
        method: "POST",
        body: formData,
      });
      if (!response.ok) {
  ...
  const handleRiceSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!riceFile) {
      alert("Please select a rice leaf image");
      return;
    }
    setRiceLoading(true);
    try {
      ...
      if (!res.ok) {
        ...
        setRiceResult({
          success: false,
          error: errData.error || `Server error (${res.status}). Please try again.`,
        });
        return;
      }
  ...
  const speakResult = (data: any) => {
    ...
    const cleanInfo = (data.disease_info || "")
      .replace(/<[^>]*>/g, " ")
      .replace(/\s+/g, " ")
      .trim();
    ...
    <div className="min-h-screen bg-eco-cream py-20">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <p className="text-gray-600 mb-6">
          Upload plant leaf images for accurate disease detection and analysis
        </p>
                  ...
                  <div className="border-2 border-dashed border-eco-green rounded-lg p-8 text-center">
                    ...
                    <label htmlFor="rice-input" className="cursor-pointer">
                      ...
                      <p className="text-gray-500 text-sm mt-1">
                        Detects: Bacterial leaf blight · Brown spot · Leaf smut
                      </p>
                    </label>
                    {riceFile && (
                      <p className="mt-3 text-eco-green font-semibold">✅ {riceFile.name}</p>
                    )}
                  </div>
                </div>
                <button
                  type="submit"
                  disabled={riceLoading || !riceFile}
              ...
              <div className="mt-8">
                {!riceResult.success ? (
                  ...
                  <div className="space-y-4">
                    <div
                      className={`p-6 rounded-lg border-2 ${getDiseaseColor(riceResult.disease)}`}
                    >
                      ...
                      <p className="text-sm text-gray-600 mb-3">
                        Confidence: <strong>{riceResult.confidence}%</strong>
                      </p>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                      ...
                      {Object.entries(riceResult.all_probabilities).map(
                        ([cls, prob]: [string, any]) => (
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-blue-50 p-4 rounded-lg border-2 border-blue-300">
                  <p className="text-blue-700 text-sm">
                    <span className="font-semibold">💡 Tip:</span> Upload multiple images for better
                    accuracy. The system analyzes each image and identifies the most common disease.
                  </p>
                </div>
                ...
                >
                  {loading
                    ? "⏳ Analyzing Images..."
                    : `🔍 Detect Disease (${uploadedFiles.length})`}
                </button>
              </form>
            </div>
                  ...
                  <div>
                    ...
                    <div className="bg-blue-50 p-6 rounded-lg border-2 border-blue-300 mb-6">
                      <p className="text-blue-700 text-lg">
                        <span className="font-semibold">📊 Summary:</span> Analyzed{" "}
                        {result.total_images} image(s). Most common:{" "}
                        <strong>
                          {(() => {
                    ...
                    <div
                      className={`p-6 rounded-lg border-2 mb-6 ${getDiseaseColor(result.most_common_disease)}`}
                    >
                      <h3 className="text-lg font-bold text-gray-800 mb-3">
                      ...
                      <div
                        ...
                        dangerouslySetInnerHTML={{
                          __html: DOMPurify.sanitize(result.disease_info ?? ""),
                        }}
                      />
                    </div>
                    ...
                    <div className="mb-6">
                      ...
                      <div className="space-y-3">
                        {result.predictions.map((pred: any, idx: number) => (
                          <div
                            key={idx}
                            className={`p-4 rounded-lg border-2 ${getDiseaseColor(pred.disease || "")}`}
                          >
                            ...
                            {pred.success ? (
                              <div className="mt-2 flex flex-wrap gap-2">
                                {(() => {
                                  const { crop, disease } = parseDiseaseName(pred.disease);
                                  return (
                                    <>
                                      <span className="bg-white border border-gray-300 rounded px-2 py-0.5 text-xs text-gray-600">
                                        🌿 {crop}
                                      </span>
                                      <span className="bg-white border border-gray-300 rounded px-2 py-0.5 text-xs text-gray-600">
                                        🦠 {disease}
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-6">
                      ...
                      <div className="bg-blue-50 p-4 rounded-lg border-2 border-blue-300 text-center">
                        ...
                        <p className="text-3xl font-bold text-blue-600 mt-1">
                          {result.predictions.filter((p: any) => p.success).length}
                        </p>
                      </div>
